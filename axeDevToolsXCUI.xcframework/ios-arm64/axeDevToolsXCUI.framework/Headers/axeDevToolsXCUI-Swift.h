...
/// Auto Scan control surface for Appium/WebDriverAgent.
/// Mirrors Android’s <code>AxeAppium</code> (<code>pauseAutoScan()</code> / <code>resumeAutoScan()</code>) so the downstream
/// Appium integration is call-site identical across platforms. Not intended for direct
/// in-test use — test authors should prefer <code>AxeAutoScan.skipScan { }</code>, which cannot be
/// left dangling.
SWIFT_CLASS("_TtC15axeDevToolsXCUI9AxeAppium")
@interface AxeAppium : NSObject
/// Creates a control surface bound to the live Auto Scan session (<code>AutoScanManager.shared</code>).
- (nonnull instancetype)init;
/// Pauses Auto Scan capture. Gestures still execute normally; their screens are not captured.
/// Idempotent: calling while already paused is a no-op. Always pair with <code>resumeAutoScan()</code>.
- (void)pauseAutoScan;
/// Resumes Auto Scan capture after a pause.
/// Idempotent: calling while not paused is a no-op.
- (void)resumeAutoScan;
@end

/// Customer-facing entry point for excluding interactions from Auto Scan.
/// Auto Scan is enabled the usual way (via <code>axe_config.json</code>); this API only adjusts capture
/// behavior while a run is already in progress. Matches Android’s static
/// <code>AxeAutoScan.skipScan { }</code> so cross-platform call sites are identical.
SWIFT_CLASS("_TtC15axeDevToolsXCUI11AxeAutoScan")
@interface AxeAutoScan : NSObject
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
/// \param apiKey API Key for axeDevTools provided by Deque Systems.
///
/// \param axeAccountUrl URL of the axe account instance. Only needed for enterprises with their own instance. Defaults to empty string (uses default server).
///
///
/// throws:
/// An error if the API key is invalid or authentication fails.
...
/// returns:
/// An initialized AxeDevTools object ready for local scanning.
+ (AxeDevTools * _Nullable)authenticateWithApiKey:(NSString * _Nonnull)apiKey axeAccountUrl:(NSString * _Nonnull)axeAccountUrl error:(NSError * _Nullable * _Nullable)error SWIFT_WARN_UNUSED_RESULT;
@end
